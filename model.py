...
def main():
    ...
    training_epochs = 500
    batch_size = 128
    X = tf.placeholder(dtype=tf.float32, shape=[None, 3], name='factors')
    ...
    weights = {
        'h1' : tf.Variable(initializer([3, 128])),
    ...
    loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=Y_one_hot)) + 0.0008 * reg2
